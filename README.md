...
Claws-Defender 是一个放在 OpenClaw `extensions/` 目录中使用的本地插件仓库，不是独立 npm 包。
...
这是一个本地安装插件。请把仓库放到 OpenClaw 的 `extensions/` 目录下，再从本地路径安装。
...
Claws-Defender is a local OpenClaw runtime security plugin repository intended to live under the OpenClaw `extensions/` directory. It is not a standalone npm package.
...
This plugin is installed locally from source. Place the repository under your OpenClaw `extensions/` directory, build it there, and install it from the local path.
